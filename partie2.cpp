...
void add_if_top(std::vector<circleToDetect> *circleVector, circleToDetect c, int nb_circles)
{
...
int main(int argc, char** argv ) {
  ...
  // Contour des cercles des images en couleurs
  for(int r = 0; r < acc.size(); r++) {
		for(int c = 0; c < acc[0].size(); c++) {
            ...
            add_if_top(circleWithHighestScore, cTd, nbCerclesToDetect);
				 }
			}
		}
	}
  ...
  cv::imshow( "Detection de Cercle -- Four", image);
  cv::imshow( "Detection de Cercle -- Coins", image_coins);
  cv::imshow( "Detection de Cercle -- Filtre Sobel", gradient);
